...
/**
 * Tela dev (JSON colado / entrada manual). Preservada como escape hatch pra
 * emergência ou teste técnico. Gated pela senha `010894` — usuário final não
 * cai aqui por acidente. Tela do usuário final vive em /emitir.
 */
const SENHA_ADMIN_ACCESS = '010894';

function GateSenhaAdmin({ children }: { children: React.ReactNode }) {
  ...
  function tenta() {
    if (senha === SENHA_ADMIN_ACCESS) { setLiberado(true); return; }
  ...
  return (
    <div className="min-h-[80vh] grid place-items-center bg-bg p-6">
      <div className="bg-card rounded-DEFAULT shadow p-7 w-[min(440px,100%)]">
        <div className="w-12 h-12 rounded-xl bg-bg grid place-items-center mb-4">
          <Lock size={22} />
        </div>
        <h4 className="text-xl font-extrabold tracking-tight mb-2">Tela técnica</h4>
        <p className="text-sm text-ink-soft mb-4">
          Modo JSON / entrada manual é restrito à equipe. Se você é operador da loja, use a{' '}
          <a href="/emitir" className="text-i9 underline">tela de emissão normal</a>.
        </p>
        <input
          ref={ref}
          type="password"
          ...
          autoComplete="off"
        />
        {erro && <div className="text-warn text-xs mt-2 text-center">{erro}</div>}
        <button
          onClick={tenta}
          className="mt-4 w-full py-3 rounded-lg bg-ink text-white font-extrabold text-sm"
        >
          Entrar
        </button>
      </div>
...
export default function EmitirNotaAdminGated() {
  return <GateSenhaAdmin><EmitirNotaAdminInterna /></GateSenhaAdmin>;
}

function EmitirNotaAdminInterna() {
  ...
  // Endereço do destinatário — obrigatório em NF-e mod 55 (SEFAZ cStat 726).
  // Em NFC-e (mod 65) é opcional; se ficar tudo em branco, o backend omite o bloco.
  ...
  // Monta o objeto cliente (com endereço opcional) a partir dos campos manuais.
  // Só inclui `endereco` se pelo menos um campo tiver sido preenchido.
  const montarCliente = () => {
    if (!clienteNome && !clienteCpf) return undefined;
    const cliente: any = { nome: clienteNome, cpf: clienteCpf };
    const temEndereco = [clienteLogradouro, clienteNumero, clienteBairro, clienteCidade, clienteUf, clienteCep]
      .some(v => v.trim());
    if (temEndereco) {
      cliente.endereco = {
        ...
        uf: clienteUf.trim().toUpperCase(),
        cep: clienteCep.replace(/\D/g, ''),
      };
    }
    return cliente;
  };
  ...
  }, [modoEntrada, clienteNome, clienteCpf, clienteLogradouro, clienteNumero, clienteBairro, clienteCidade, clienteUf, clienteCep, descontoManual, meioPagamento, itensManuais]);
          ...
          {modoEntrada === 'json' ? (
            ...
            <div className="flex flex-col gap-6">
              ...
              <div className="bg-card border border-line rounded-DEFAULT shadow p-5 flex flex-col gap-4">
                <h3 className="text-xs font-extrabold text-i9 uppercase tracking-wider border-b border-line pb-2">Identificação do Consumidor</h3>
                <p className="text-[10px] text-muted -mt-2">
                  NFC-e (65): opcional. NF-e (55): obrigatório — CPF/CNPJ + endereço completo.
                </p>
                ...
                {/* Endereço do destinatário — obrigatório em NF-e mod 55 */}
                <div className="border-t border-line-soft pt-4 flex flex-col gap-3">
                  <h4 className="text-[10px] font-extrabold text-muted uppercase tracking-wider">Endereço (obrigatório em NF-e)</h4>
                  <div className="grid grid-cols-1 sm:grid-cols-6 gap-3">
                    <div className="flex flex-col gap-1.5 sm:col-span-4">
                      <label className="text-[10px] font-bold text-muted uppercase">Logradouro</label>
                      <input
                        type="text"
                    ...
                    <div className="flex flex-col gap-1.5 sm:col-span-2">
                      <label className="text-[10px] font-bold text-muted uppercase">Número</label>
                      <input
                        type="text"
                        value={clienteNumero}
                    ...
                    <div className="flex flex-col gap-1.5 sm:col-span-3">
                      <label className="text-[10px] font-bold text-muted uppercase">Bairro</label>
                      <input
                        type="text"
                        value={clienteBairro}
                    ...
                    <div className="flex flex-col gap-1.5 sm:col-span-3">
                      <label className="text-[10px] font-bold text-muted uppercase">CEP</label>
                      <input
                        type="text"
                        value={clienteCep}
                    ...
                    <div className="flex flex-col gap-1.5 sm:col-span-4">
                      <label className="text-[10px] font-bold text-muted uppercase">Cidade</label>
                      <input
                        type="text"
                        value={clienteCidade}
                    ...
                    <div className="flex flex-col gap-1.5 sm:col-span-2">
                      <label className="text-[10px] font-bold text-muted uppercase">UF</label>
                      <input
                        type="text"
                        maxLength={2}
                        value={clienteUf}
                        ...
                        placeholder="MG"
                        className="bg-field border border-line rounded-lg px-3 py-2 text-sm focus:border-i9 outline-none uppercase font-mono"
                      />
                    </div>
                  </div>
                </div>
